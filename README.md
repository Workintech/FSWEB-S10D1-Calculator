...
Geçen hafta yaptığın GridGame beğenildi. Yarıda kalmış bir uygulamayı tmamlaman istendi.
...
- [ ] Uygulama klasöründe gezin ve özellikle "App.jsx", "/store/reducers/reducers.jsx" ve "/store/actions/actions.jsx" dosyalarına bakın.
...
- [ ] App.jsx içine, useReducer hookunu import edin, `./store/reducers/reducers.jsx`dan reducerı ve initialState'i kullanarak tanımlayın.
...
- [ ] `APPLY_NUMBER` actionını (./store/reducers/reducers.jsx içindeki) ve `applyNumber` action creatorı (./store/actions/actions.jsx içindeki) alalım. Bu action, action creatora iletilen bir sayıyı ekler, çarpar veya çıkarır.
...
- [ ] `CHANGE_OPERATION` actionı (`/store/reducers/reducers.jsx` içinde) alın. Bu reducer operatör değeri alır (+,- ve \*) ve bunu state'e aktarır.
- [ ] Argümanı olarak bir operatör alan bir action creator oluşturun (`store/actions/actions.jsx` içinde) ve `CHANGE_OPERATION` tipinde bir action constant oluşturun.
- [ ] Yeni action creatorı `App.jsx`e import edin
...
- [ ] `/store/reducers/reducers.jsx` içinde, `CLEAR_DISPLAY` casei oluşturun. Bu case total değer statini 0 yapacak.
- [ ] `/store/actions/actions.jsx` içine, action creator ve `CLEAR_DISPLAY` action constantı oluşturun. (Reducer dosyanızda bu sabiti import ettiğinizden emin olun.)
